...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
  ...
  public void configure(HttpSecurity http) throws Exception {
    http.exceptionHandling()
        ...
        .and()
        .requestMatcher(new OAuthRequestedMatcher(uaaProperties.getSecurity()))
        .authorizeRequests()
        .anyRequest()
        .fullyAuthenticated();
  }
  ...
  private static class OAuthRequestedMatcher implements RequestMatcher {
    private final UaaProperties.Security security;

    public boolean matches(HttpServletRequest request) {
      String auth = request.getHeader(security.getAuthorization().getHeader());
      boolean haveOauth2Token =
          (auth != null) && auth.startsWith(security.getJwt().getTokenPrefix());
